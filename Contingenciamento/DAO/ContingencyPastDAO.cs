...
namespace Contingenciamento.DAO
{
    public class ContingencyPastDAO
    {
        ...
        /*
         * Antes temos que buscar na tabela de EmployeeHistory cada registro que origina os registros na tabela ContingencyPast. 
         * Cada registro de EmployeeHistory origina 'n' registros em ContingencyPast, onde 'n' é o número de verbas utilizadas (férias, 13º, incidência).
         * Aí esse método atualiza o registro em EmployeeHistory para flag de processed ser FALSE novamente e depois apaga os registros em ContingencyPast
         */
        public int DeleteContingencyByContractAndDateRange(Contract ct, DateTime start, DateTime end)
        {
            int rowsAffected = -1;
            int rowsDelAffected = -1;
            NpgsqlDataReader reader = null;
            List<long> empHistoryIds = new List<long>();
            try
            {
                string startStr = start.ToString("yyyy-MM-dd");
                string endStr = end.ToString("yyyy-MM-dd");
                string cmdUpdate = "UPDATE employee_history SET \"processed\" = false " +
                    "WHERE \"contract_id\" = '" + ct.Id + "' AND \"epoch\" BETWEEN ('"+startStr+"') AND ('"+endStr+"');";

                string cmdSelect = "SELECT id FROM employee_history WHERE contract_id = "+ct.Id+" AND epoch BETWEEN ('"+startStr+"') AND ('"+endStr+"');";
                string cmdDelete = "DELETE FROM contingency_past WHERE employee_history_id IN ";
                
                dal.OpenConnection();

                NpgsqlCommand cmd1 = new NpgsqlCommand(cmdUpdate);                
                rowsAffected = dal.ExecuteNonQuery(cmd1);

                NpgsqlCommand cmd2 = new NpgsqlCommand(cmdSelect);
                reader = dal.ExecuteDataReader(cmd2);
                String ehIdsListStr = "";
                int count = 0;
                while (reader.Read())
                {
                    empHistoryIds.Add(Convert.ToInt64(reader["id"]));
                    ehIdsListStr += (count == 0) ? reader["id"].ToString() : "," + reader["id"].ToString();
                    count++;
                }
                reader.Close();

                if (empHistoryIds.Count > 0)
                {
                    cmdDelete += "(" + ehIdsListStr + ");";
                    NpgsqlCommand cmd3 = new NpgsqlCommand(cmdDelete);
                    rowsDelAffected = dal.ExecuteNonQuery(cmd3);
                }
            }
            catch (Exception ex)
            {
        ...
        public int DeleteContingencyByContract(Contract ct)
        {
            ...
            try
            {
                ...
                string cmdSelect = "SELECT id FROM employee_history WHERE contract_id = " + ct.Id;
                string cmdDelete = "DELETE FROM contingency_past WHERE employee_history_id IN ";
